...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(100, 100, WINDOW_LENGTH, WINDOW_HIEGHT)
        self.setWindowTitle("Matrix Mask Picker")

        self.label_calib_data = QtWidgets.QLabel(self)
        self.label_file_data = QtWidgets.QLabel(self)
        self.btn_open_json = QtWidgets.QPushButton(self)
        self.btn_open_csv = QtWidgets.QPushButton(self)
    ...
    def init_UI(self):
        ...
        self.label_calib_data.setText("Calib Data")
        self.label_calib_data.move(310, 20)

        # Set font for the label
        font = QtGui.QFont()
        font.setPointSize(14)
        self.label_calib_data.setFont(font)

        self.label_file_data.setText("File Data")
        self.label_file_data.move(1200, 20)

        # Set font for the label
        font = QtGui.QFont()
        font.setPointSize(14)
        self.label_file_data.setFont(font)

        # QPushButton for opening a JSON file
        self.btn_open_json.setText("Open JSON File")
        self.btn_open_json.move(300, 60)
        self.btn_open_json.clicked.connect(self.open_file)

        # QPushButton for opening a CSV file
        self.btn_open_csv.setText("Open CSV File")
        self.btn_open_csv.move(1195, 60)
        ...
        self.btn_actuators_mask.move(230, 100)
        ...
        self.btn_relevant_lenslets.move(380, 100)
        ...
        self.actuators_mask_table.setHorizontalHeaderLabels(
        ...
        self.output_mask_table.setHorizontalHeaderLabels(
            ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l']
        )
    ...
    def setup_table(self, table, rows, columns, x, y):
        # Set up the table geometry and headers
        ...
        if not self.csv_data:
            ...
            for i in range(rows):
                for j in range(columns):
                    ...
                    # TODO - abstract these colors into global variables
                    # Set the cell color
                    if value == 1:
    ...
    # TODO - def save_scv_data_as_table(data, row_num):
    def save_table(self):
        if not self.csv_data:
            self.show_popup("Please load a CSV file first.")
            return
    ...
    def set_cell_color(self, item, new_value, true_color, false_color):
        if new_value == 1:
            item.setBackground(true_color)  # Green
        else:
            item.setBackground(false_color)  # Blue        
